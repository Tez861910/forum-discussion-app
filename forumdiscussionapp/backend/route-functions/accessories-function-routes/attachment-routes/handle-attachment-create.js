...
async function handleAttachmentCreate(req, res) {
  ...
  try {
    ...
    const [result] = await query(sql, [filePath, attachedByUserId, attachmentTypeId, attachedToType, attachedToId, description]);
    ...
    res.status(500).json({ error: 'Attachment creation failed', details: error.message });
  }
}
