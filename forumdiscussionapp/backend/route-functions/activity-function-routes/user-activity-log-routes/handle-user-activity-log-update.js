...
async function handleUserActivityLogUpdate(req, res) {
  ...
  try {
    ...
    const [result] = await query(sql, [userId, activityType, activityDetails, ipAddress, logId]);
    ...
    res.status(500).json({ error: 'User activity log update failed', details: error.message });
  }
}
