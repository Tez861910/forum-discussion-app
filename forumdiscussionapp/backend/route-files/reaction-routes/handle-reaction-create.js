...
async function handleReactionCreate(req, res) {
  ...
  try {
    ...
    const [result] = await query(sql, [userId, reactionTypeId, reactedToType, reactedToId, isPositive]);
    ...
    res.status(500).json({ error: 'Reaction creation failed', details: error.message });
  }
}
