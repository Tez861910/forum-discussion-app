...
async function handleUserReportUpdate(req, res) {
  ...
  try {
    if (!reporterId || !reportedUserId || !reportContent) {
      ...
      return res
        .status(400)
        .json({ error: 'ReporterId, ReportedUserId, and ReportContent are required' });
    }

    const sql =
      'UPDATE UserReports SET ReporterID = ?, ReportedUserID = ?, ReportContent = ?, IsResolved = ? WHERE ReportID = ?';
    const [result] = await query(sql, [reporterId, reportedUserId, reportContent, isResolved, reportId]);
    ...
    res.status(500).json({ error: 'User report update failed', details: error.message });
  }
}
