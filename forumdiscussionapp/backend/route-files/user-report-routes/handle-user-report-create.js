...
async function handleUserReportCreate(req, res) {
  ...
  try {
    ...
    const [result] = await query(sql, [reporterId, reportedUserId, reportContent]);
    ...
    res.status(500).json({ error: 'User report creation failed', details: error.message });
  }
}
