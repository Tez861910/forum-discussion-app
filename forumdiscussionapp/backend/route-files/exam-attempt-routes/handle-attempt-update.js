...
async function handleAttemptUpdate(req, res) {
  const { attemptId } = req.params;
  const { userId, examId, startTime, endTime, status } = req.body;

  try {
    const sql = 'UPDATE ExamAttempt SET UserID = ?, ExamID = ?, StartTime = ?, EndTime = ?, Status = ? WHERE AttemptID = ?';
    const [result] = await query(sql, [userId, examId, startTime, endTime, status, attemptId]);
    ...
    res.status(500).json({ error: 'Exam attempt update failed', details: error.message });
  }
}
