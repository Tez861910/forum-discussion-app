...
async function handleUserSettingsCreate(req, res) {
  ...
  try {
    ...
    const [result] = await query(sql, [userId, theme, darkMode, language, emailNotifications]);
    ...
    res.status(500).json({ error: 'User settings creation failed', details: error.message });
  }
}
