...
async function editRecurringEvent(req, res) {
  try {
    ...
    await query(
      ...
      [UpdatedRecurrenceType, UpdatedRecurrenceInterval, eventId, recurringEventId]
    );
    ...
    res.json({ success: true, message: 'Recurring event updated successfully' });
  } catch (error) {
    console.error('Error updating recurring event:', error);
    res.status(500).json({ success: false, error: 'Error updating recurring event' });
  }
}
