...
async function handleUpdateCreate(req, res) {
  const { userId, updateType, data } = req.body;

  try {
    const sql = 'INSERT INTO RealTimeUpdates (UserID, UpdateType, Data) VALUES (?, ?, ?)';
    ...
    res.status(500).json({ error: 'Real-time update creation failed', details: error.message });
  }
}
