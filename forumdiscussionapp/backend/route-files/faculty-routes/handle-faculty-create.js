...
async function handleFacultyCreate(req, res) {
  const { facultyName, departmentId } = req.body;

  try {
    const sql = 'INSERT INTO FacultyMembers (FacultyName, DepartmentID) VALUES (?, ?)';
    ...
    res.status(500).json({ error: 'Faculty member creation failed', details: error.message });
  }
}
