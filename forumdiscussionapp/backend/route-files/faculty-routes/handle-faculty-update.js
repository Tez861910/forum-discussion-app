...
async function handleFacultyUpdate(req, res) {
  const { facultyId } = req.params;
  const { facultyName, departmentId } = req.body;

  try {
    const sql = 'UPDATE FacultyMembers SET FacultyName = ?, DepartmentID = ? WHERE FacultyID = ?';
    ...
    res.status(500).json({ error: 'Faculty member update failed', details: error.message });
  }
}
