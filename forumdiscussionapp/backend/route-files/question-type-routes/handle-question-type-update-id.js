...
async function handleQuestionTypeUpdateById(req, res) {
  const { questionTypeId } = req.params;
  const { questionTypeName } = req.body;

  try {
    const sql = 'UPDATE QuestionType SET QuestionTypeName = ? WHERE QuestionTypeID = ?';
    ...
    res.status(500).json({ error: 'Question type update failed', details: error.message });
  }
}
