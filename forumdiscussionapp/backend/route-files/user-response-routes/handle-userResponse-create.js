...
async function handleUserResponseCreate(req, res) {
  ...
  try {
    ...
    const sql = 'INSERT INTO UserResponses (UserID, QuestionID, AnswerID) VALUES (?, ?, ?)';
    ...
    res.status(500).json({ error: 'User response creation failed', details: error.message });
  }
}
