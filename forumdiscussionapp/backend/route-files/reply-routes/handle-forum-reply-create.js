...
async function handleForumReplyCreate(req, res) {
  ...
  try {
    if (!userId || !replyContent) {
      ...
      return res.status(400).json({ error: 'UserID and ReplyContent are required' });
    }

    const sql = 'INSERT INTO ForumsReplies (ForumPostID, UserID, ReplyContent) VALUES (?, ?, ?)';
    ...
    res.status(500).json({ error: 'Forum reply creation failed', details: error.message });
  }
}
