...
async function handleQuestionUpdateById(req, res) {
  ...
  try {
    ...
    const [result] = await query(sql, [questionText, questionTypeId, examId, courseId, createdByUserId, questionId]);
    ...
    res.status(500).json({ error: 'Question update failed', details: error.message });
  }
}
