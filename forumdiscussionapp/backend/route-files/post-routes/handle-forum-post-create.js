...
async function handleForumPostCreate(req, res) {
  ...
  try {
    if (!userId || !postContent) {
      ...
      return res.status(400).json({ error: 'UserID and PostContent are required' });
    }

    const sql = 'INSERT INTO ForumsPosts (ForumID, UserID, PostContent) VALUES (?, ?, ?)';
    ...
    res.status(500).json({ error: 'Forum post creation failed', details: error.message });
  }
}
